...
DirHiddenRequest::DirHiddenRequest(const std::string& driveID, const std::string& fileID, bool hidden):
        driveID_(driveID),
        fileID_(fileID),
        hidden_(hidden)
{
...
std::shared_ptr<std::iostream> DirHiddenRequest::Body() const
{
    Json::Value root;
...
int DirHiddenRequest::validate() const
{
    return 0;
}
