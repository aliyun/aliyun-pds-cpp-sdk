...
DirCopyRequest::DirCopyRequest(const std::string& driveID, const std::string& fileID, const std::string& toParentFileID,
    bool autoRename):
        driveID_(driveID),
        fileID_(fileID),
...
std::shared_ptr<std::iostream> DirCopyRequest::Body() const
{
    Json::Value root;
...
int DirCopyRequest::validate() const
{
    return 0;
}
