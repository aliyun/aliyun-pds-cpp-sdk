...
FileGetVideoPreviewPlayInfoResult::FileGetVideoPreviewPlayInfoResult():
        PdsResult()
{
}
        ...
        const std::shared_ptr<std::iostream>& content)
{
    ...
    shareID_ = root["share_id"].asString();
    driveID_ = root["drive_id"].asString();
    fileID_ = root["file_id"].asString();

    Json::Value videoInfo = root["video_preview_play_info"];
    fileVideoPreviewPlayInfo_.ParseFromJson(videoInfo);
}
