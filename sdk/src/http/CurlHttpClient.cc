...
#include <alibabacloud/pds/client/RateLimiter.h>
...
namespace PDS
{
    ...
    static int debugCallback(void *handle, curl_infotype type, char *data, size_t size, void *userp)
    {
        ...
        default:
            break;
        case CURLINFO_TEXT:
            PDS_LOG(LogLevel::LogInfo, TAG, "handle(%p)=> Info: %.*s", handle, size, data);
            break;
        case CURLINFO_HEADER_OUT:
            PDS_LOG(LogLevel::LogDebug, TAG, "handle(%p)=> Send header: %.*s", handle, size, data);
            break;
        case CURLINFO_HEADER_IN:
            PDS_LOG(LogLevel::LogDebug, TAG, "handle(%p)=> Recv header: %.*s", handle, size, data);
            break;
        }
        return 0;
    }
...
{
    PDS_LOG(LogLevel::LogDebug, TAG, "request(%p) enter makeRequest", request.get());
    ...
    PDS_LOG(LogLevel::LogDebug, TAG, "request(%p) acquire curl handle:%p", request.get(), curl);
    ...
    if (body != nullptr) {
        ...
        if (res != CURLE_OK && transferState.recvBodyPos != static_cast<std::streampos>(-1)) {
            PDS_LOG(LogLevel::LogDebug, TAG, "request(%p) setResponseBody, tellp:%lld, recvBodyPos:%lld",
    ...
    PDS_LOG(LogLevel::LogDebug, TAG, "request(%p) leave makeRequest, CURLcode:%d, ResponseCode:%d",
