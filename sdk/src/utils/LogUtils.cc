...
static LogLevel    gPdsLogLevel = LogLevel::LogOff;
...
void AlibabaCloud::PDS::FormattedLog(LogLevel logLevel, const char* tag, const char* fmt, ...)
{
    std::stringstream ss;
...
LogLevel AlibabaCloud::PDS::GetLogLevelInner()
{
    return gPdsLogLevel;
}

LogCallback AlibabaCloud::PDS::GetLogCallbackInner()
{
    return gLogCallback;
}

void AlibabaCloud::PDS::SetLogLevelInner(LogLevel level)
{
    gPdsLogLevel = level;
}

void AlibabaCloud::PDS::SetLogCallbackInner(LogCallback callback)
{
    gLogCallback = callback;
}

void AlibabaCloud::PDS::InitLogInner()
{
    gPdsLogLevel = LogLevel::LogOff;
    gLogCallback = nullptr;
    auto value = std::getenv("PDS_SDK_LOG_LEVEL");
    if (value) {
        ...
        for (auto i = 0U; i < size; i++) {
            if (level.compare(EnvLogLevels[i]) == 0) {
                gPdsLogLevel = static_cast<decltype(LogLevel::LogOff)>(static_cast<decltype(i)>(LogLevel::LogOff) + i);
...
void AlibabaCloud::PDS::DeinitLogInner()
{
    gPdsLogLevel = LogLevel::LogOff;
    gLogCallback = nullptr;
}
