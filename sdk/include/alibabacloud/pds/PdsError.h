...
namespace PDS
{
    class ALIBABACLOUD_PDS_EXPORT PdsError
    {
    public:
        PdsError() = default;
        PdsError(const std::string& code, const std::string& message) :
            code_(code),
            message_(message)
        {
        }
        PdsError(const PdsError& rhs) :
            code_(rhs.code_),
        ...
        PdsError& operator=(PdsError&& lhs)
        {
            code_ = std::move(lhs.code_);
            message_ = std::move(lhs.message_);
            requestId_ = std::move(lhs.requestId_);
        ...
        PdsError& operator=(const PdsError& rhs)
        {
            code_ = rhs.code_;
            message_ = rhs.message_;
            requestId_ = rhs.requestId_;
